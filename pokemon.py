'''
This file is a modified version of the original pokemon.py file from the
poke_battle_sim library. It includes a new method `fast_copy()` to create
a copy of a Pokemon object without the overhead of the __init__ method,
which is crucial for performance in AI simulations such as Minimax.

------------------------------------------------------------

WHAT TO DO?
You need to substitute this file in the library in order to add the fast_copy() method and other adjustments.
'''
...
class Pokemon:
    ...
    def fast_copy(self) -> Pokemon:
        """
        Creates a copy of the Pokemon without the overhead of __init__.
        Crucial for AI simulations (Minimax).
        """
        # Create a new, empty instance of Pokemon
        new_poke = Pokemon.__new__(Pokemon)
        
        # 1. Shallow copy the entire dictionary (fastest way to copy scalars like level, id, name)
        new_poke.__dict__.update(self.__dict__)
        
        # 2. Deep copy only the mutable lists/objects that change during battle
        new_poke.stats_actual = self.stats_actual[:]
        new_poke.stats_effective = self.stats_effective[:]
        new_poke.stat_stages = self.stat_stages[:]
        new_poke.v_status = self.v_status[:]
        
        # Deep copy moves so PP/Disable changes in sim don't affect real pokemon
        new_poke.moves = [m.get_tcopy() for m in self.moves]
        # o_moves can just be shallow copied (reference list) as they are read-only blueprints
        new_poke.o_moves = [m.get_tcopy() for m in self.o_moves]
        
        # Handle the volatile next_moves Queue
        new_poke.next_moves = Queue()
        # (Optional: If you need to support mid-Turn moves like Outrage in sims, 
        # you would need to manually copy items from self.next_moves here, 
        # but for a general speedup, an empty queue is usually an acceptable compromise)

        # 3. Clear references to the old battle/trainer to prevent tangling
        new_poke.cur_battle = None
        new_poke.trainer = None
        new_poke.enemy = None
